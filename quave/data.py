...
class DatasetKvasir(Dataset):
    ...
    def __init__(
        ...
        if images_dir == "/var/datasets/Kvasir":
            ...
            self.train_X, self.validation_X = train_test_split(
                path_images,
                test_size=0.25,
                train_size=0.75,
                random_state=seed,
...
class DatasetForCAE_IXI(torch.utils.data.Dataset):
    ...
    def __init__(self, path, train, transform, seed=42):

        # path = "./IXI_preprocess_dataset"

        path1 = (
            "../../data/IXI_preprocess_dataset/IXI-T1/T1.npz"  # path + "/IXI-T1/T1.npz"
        )
        path2 = (
            "../../data/IXI_preprocess_dataset/IXI-T2/T2.npz"  # path + "/IXI-T2/T2.npz"
        )

        self.images1 = np.load(path1, allow_pickle=True)[
            "arr_0"
        ]  # array of uint8 (581, 256,256) valori [0,255]
        self.images2 = np.load(path2, allow_pickle=True)[
            "arr_0"
        ]  # array of uint8 (578, 256,256) valori [0,255]

        self.train, self.validation = train_test_split(
            self.images1, test_size=0.25, train_size=0.75, random_state=seed
        )  # train (435,256,256)  validation (146,256,256)
        self.train2, self.validation2 = train_test_split(
            self.images2, test_size=0.25, train_size=0.75, random_state=seed
        )  # train2 (433,256,256)
        ...
        if train:
            ...
            self.input = np.concatenate(
                (self.validation, self.validation2), axis=0
            )  # (290, 256, 256)
            self.input = self.input[np.random.permutation(self.input.shape[0]), :]
...
class DIV2KDataset(Dataset):
    def __init__(
        ...
        super().__init__()
        # Collect all HR images
        self.paths = sorted(
            list(Path(os.path.join(data_dir, f"DIV2K_{subset}_HR")).glob("*.png"))
        )

        # Define transforms with resizing to ensure all images are the same size
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(img_path).convert("RGB")  # or 'L' if grayscale is needed
