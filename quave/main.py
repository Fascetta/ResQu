...
if __name__ == "__main__":
    ...
    parser.add_argument("--wavelet_type", type=str, default="qwt", help="")
    ...
    model = ImageFusionNetworkPL(
        in_channels=1 if args.dataset == "ixi" else 3,
    ...
    if args.dataset == "ixi":
        ...
        ixi_data = DIV2KDataModule(
            data_dir="../../data/DIV2K", batch_size=32, seed=args.seed
        )
    else:
...
class WaveletWeights(object):
    ...
    def __init__(self, wav_type, device):
        ...
        self.device = device
        str_path = (
            "../pretrained_weights/IFN-IXI-epoch=399-psnr=28.73.ckpt"
            if self.wav_type == "qwt"
            else "../pretrained_weights/IFN-IXI-DWT-epoch=399-psnr=24.80.ckpt"
        )
        self.model_IF = (
            ImageFusionNetworkPL.load_from_checkpoint(str_path).to(self.device).eval()
        )
    ...
    def __call__(self, sample):
        ...
        self.model_IF.WTM.last_weights.register_forward_hook(
            self.get_activation(return_string, activation_json)
        )
