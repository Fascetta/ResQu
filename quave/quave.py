...
from data import CelebADataset, DatasetForCAE_IXI, DatasetKvasir
...
class ResidualGroup(nn.Module):
    def __init__(self, n_feat, out_channels, kernel_size, n_resblocks=3):
        ...
        modules_body = [
            ResidualBlock(
                n_feat=n_feat, out_channels=out_channels, kernel_size=kernel_size
            )
...
class RoughFeatureExtractionModule(nn.Module):
    def __init__(
        self, in_channels=3, out_channels=64, kernel_size=3, n_residual_groups=3
    ):
...
class WaveletTransformModule(nn.Module):
    def __init__(
        ...
        self.split_size = (
            out_channels * 1 if self.wavelet_type == "dwt" else out_channels * 4
        )
        self.wavelet_layer = (
            DWTForward(J=1, wave="bior1.3", mode="zero")
            if wavelet_type == "dwt"
            else QWTForward(device)
        )
        self.feature_refinement_ll = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )
        self.feature_refinement_lh = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )
        self.feature_refinement_hl = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )
        self.feature_refinement_hh = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )

        if self.wavelet_type == "qwt":
            self.shrink_conv0 = nn.Conv2d(
                in_channels=in_channels * 4,
                out_channels=in_channels,
                kernel_size=1,
                padding=0,
            )
            self.shrink_conv1 = nn.Conv2d(
                in_channels=in_channels * 4,
                out_channels=in_channels,
                kernel_size=1,
                padding=0,
            )
            self.shrink_conv2 = nn.Conv2d(
                in_channels=in_channels * 4,
                out_channels=in_channels,
                kernel_size=1,
                padding=0,
            )
            self.shrink_conv3 = nn.Conv2d(
                in_channels=in_channels * 4,
                out_channels=in_channels,
                kernel_size=1,
                padding=0,
        ...
        self.feature_upsample_lh = ResidualGroup(
            n_feat=in_channels * 2,
            out_channels=in_channels * 2,
            kernel_size=kernel_size,
        )
        self.feature_upsample_hl = ResidualGroup(
            n_feat=in_channels * 2,
            out_channels=in_channels * 2,
            kernel_size=kernel_size,
        )
        self.feature_upsample_hh = ResidualGroup(
            n_feat=in_channels * 2,
            out_channels=in_channels * 2,
            kernel_size=kernel_size,
        )

        self.reconstruction_ll = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )
        self.reconstruction_lh = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )
        self.reconstruction_hl = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )
        self.reconstruction_hh = ResidualGroup(
            n_feat=in_channels, out_channels=in_channels, kernel_size=kernel_size
        )

        out_chan = out_channels if self.wavelet_type == "dwt" else 4 * out_channels
        self.conv0 = nn.Conv2d(
            in_channels=in_channels, out_channels=out_chan, kernel_size=1, padding=0
        )
        self.conv1_0 = nn.Conv2d(
            in_channels=192, out_channels=64, kernel_size=kernel_size, padding=0
        )
        self.conv1_1 = nn.Conv2d(
            in_channels=in_channels, out_channels=out_chan, kernel_size=1, padding=1
        )
        ...
        out_chan_weigh = (
            out_channels * 4 if self.wavelet_type == "dwt" else out_channels * 16
        )
        self.last_weights = nn.Conv2d(
            in_channels=out_chan_weigh, out_channels=out_chan_weigh, kernel_size=1
        )
        if self.double_last_layer:
            self.last_weights = nn.Conv2d(
                in_channels=out_chan_weigh, out_channels=4, kernel_size=1
            )
            self.qwt_last_weights = nn.Conv2d(
                in_channels=4, out_channels=out_chan_weigh, kernel_size=1
            )  # if self.wavelet_type =='qwt' else None
        ...
        self.inverse_wavelet_layer = (
            DWTInverse(wave="bior1.3", mode="zero")
            if self.wavelet_type == "dwt"
            else QWTInverse(device)
        )
    ...
    def apply_wavelet_features(self, features):
        if self.wavelet_type == "dwt":
            ...
            )  # Resize((image_size-3, image_size-3))
        ...
        LH, HL, HH = (
            (LH - LH.min()) / (LH.max() - LH.min()),
            (HL - HL.min()) / (HL.max() - HL.min()),
            (HH - HH.min()) / (HH.max() - HH.min()),
        )
        return LL, LH, HL, HH
    ...
    def forward(self, features, return_features=False):
        """
        Args:
            features (torch.Tensor): The rough feature input from RFEM
            return_features (bool): If True, returns a dict of intermediate features for analysis
        Returns:
            res (torch.Tensor): Final reconstruction
            features_dict (dict): (if return_features=True) A dictionary of intermediate results
        """
        ...
        if self.espcn:
            LH_pre_recon = self.upsample(
                self.upsample.preprocess(self.feature_upsample_lh(LH_edge))
            ).clamp(0.0, 1.0)
            HL_pre_recon = self.upsample(
                self.upsample.preprocess(self.feature_upsample_hl(HL_edge))
            ).clamp(0.0, 1.0)
            HH_pre_recon = self.upsample(
                self.upsample.preprocess(self.feature_upsample_hh(HH_edge))
            ).clamp(0.0, 1.0)
            LL_up = self.upsample(self.upsample.preprocess(LL)).clamp(0.0, 1.0)
            features_up = nn.Upsample(scale_factor=1.5)(features)
        else:
            LH_pre_recon = self.upsample(self.feature_upsample_lh(LH_edge)).clamp(
                0.0, 1.0
            )
            HL_pre_recon = self.upsample(self.feature_upsample_hl(HL_edge)).clamp(
                0.0, 1.0
            )
            HH_pre_recon = self.upsample(self.feature_upsample_hh(HH_edge)).clamp(
                0.0, 1.0
            )
        ...
        (LL, LH, HL, HH) = torch.split(
            weights, split_size_or_sections=self.split_size, dim=1
        )
        ...
        # Return final reconstruction + (optionally) dictionary of intermediate features
        if return_features:
            return res, weights
        return res
    ...
    def load_espcn(self):
        ...
        for n, p in torch.load(
            weights_file_path, map_location=lambda storage, loc: storage
        ).items():
...
class IXIDataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_dir: str = "../../data/IXI_preprocess_dataset",
        batch_size: int = 16,
        seed=888,
    ):
        super().__init__()
    ...
    def setup(self, stage=None):
        self.dataset_train = DatasetForCAE_IXI(
            self.data_dir, transform=None, train=True, seed=self.seed
        )
        self.dataset_valid = DatasetForCAE_IXI(
            self.data_dir, transform=None, train=False, seed=self.seed
        )
    ...
    def val_dataloader(self):
        return DataLoader(
            self.dataset_valid, batch_size=self.batch_size, shuffle=False, num_workers=4
        )


class KvasirDataModule(pl.LightningDataModule):
    def __init__(
        self, data_dir: str = "/var/datasets/Kvasir", batch_size: int = 16, seed=888
    ):
        super().__init__()
        self.data_dir = data_dir
        self.batch_size = batch_size
    ...
    def setup(self, stage=None):
        self.dataset_train = DatasetKvasir(
            self.data_dir, subset="train", seed=self.seed
        )
...
class CelebADataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.dataset_train, self.dataset_valid = torch.utils.data.random_split(
            full_dataset,
...
class DIV2KDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.dataset = DIV2KDataset(
            self.data_dir, image_size=self.image_size, subset=self.subset
        )
...
class ImageFusionNetworkPL(pl.LightningModule):
    ...
    def __init__(
        ...
        self.l1 = nn.L1Loss()
        self.RFEM = RoughFeatureExtractionModule(
            in_channels=in_channels, out_channels=64, kernel_size=3, n_residual_groups=3
        )
        self.WTM = WaveletTransformModule(
            ...
            double_last_layer=double_last_layer,
        )
        ...
        self.wavelet_layer = (
            DWTForward(J=1, wave="bior1.3", mode="zero")
            if self.wavelet_type == "dwt"
            else QWTForward(self.device)
        )

    def forward(self, x, return_features=False):
        features = self.RFEM(x)
        if return_features:
            out, intermediate = self.WTM(features, return_features=True)
